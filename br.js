fabric.CustomBrush = fabric.util.createClass(fabric.PencilBrush, {
    type: "CustomBrush",
    initialize(canvas, radius, hardness, alpha) {
      ...
      this.alpha = alpha;
      this.featherGradient = this.createFeatherGradient();
      this.lastX = 0;
      this.lastY = 0;
      this.lastForce = 1;
      this.updateBrushDisplay(radius, alpha);
    },
    updateBrushDisplay: function (radius, alpha) {
        this.brushCanvas = document
        .querySelector("#brush-display")
      this.brushCanvasContext = this.brushCanvas.getContext("2d");
      this.brushCanvasContext.clearRect(
        0,
        0,
        this.brushCanvasContext.canvas.width,
        this.brushCanvasContext.canvas.height
      );
      this.brushCanvasContext.fillStyle = `rgba(0, 0, 0, ${alpha})`;
      this.brushCanvasContext.beginPath();
      this.brushCanvasContext.arc(
        this.brushCanvasContext.canvas.width / 2,
        this.brushCanvasContext.canvas.height / 2,
        radius,
        0,
        2 * Math.PI
      );
    ...
    createFeatherGradient() {
      const innerRadius = Math.min(
        this.radius * this.hardness,
        this.radius - 1
      );
    ...
    feather(ctx) {
      // feather the brush
      ctx.save();
      ctx.fillStyle = this.featherGradient.toLive(ctx);
